...
db._getModel = function getModel(collection) {
  return db.getConnection().then(() => db.models[collection]);
};
...
db.get = function get(collection, id) {
  ...
  return new Promise((resolve, reject) => {
    db._getModel(collection).then(Model => {
...
db.put = function put(collection, id, data) {
  ...
  return new Promise((resolve, reject) => {
    db._getModel(collection).then(Model => {
      const doc = new Model(Object.assign({ _id: mongoose.Types.ObjectId() }, Object.assign({ id: id }, data)));
...
db.push = function push(collection, data) {
  ...
  return new Promise((resolve, reject) => {
    db._getModel(collection).then(Model => {
      const doc = new Model(Object.assign({ _id: mongoose.Types.ObjectId() }, data));
...
db.createOrUpdate = function createOrUpdate(collection, id, data) {
  ...
  return new Promise((resolve, reject) => {
    db
      ...
      .then(doc => {
        if (!doc) {
          db.put(collection, id, data).then(resolve, reject);
